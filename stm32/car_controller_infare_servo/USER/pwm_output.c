...
/* -----------------------------------------------------------------------
  ...
  TIM_TimeBaseStructure.TIM_Period = 3600;       //当定时器从0计数到999，即为1000次，为一个定时周期
  TIM_TimeBaseStructure.TIM_Prescaler = 2400;	    //设置预分频：不预分频，即为72MHz
  ...
  TIM_OCInitStructure.TIM_Pulse = CCR_Val_Offset;	   //设置跳变值，当计数器计数到这个值时，电平发生跳变
