...
__IO uint16_t ADC1_ConvertedValue; //adc 1 for front infare
__IO uint16_t ADC3_ConvertedValue; //adc 2 for back infare
...
static void ADC3_Mode_Config(void)
{
	...
	DMA_InitStructure.DMA_MemoryBaseAddr = (u32)&ADC3_ConvertedValue;//内存地址
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;
	DMA_InitStructure.DMA_BufferSize = 1;
	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;//外设地址固定
	DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Disable;  //内存地址固定
	...
	/*配置ADC时钟，为PCLK2的8分频，即9Hz*/
	RCC_ADCCLKConfig(RCC_PCLK2_Div8); 
	/*配置ADC3的通道10为55.	5个采样周期，序列为1 */ 
	ADC_RegularChannelConfig(ADC3, ADC_Channel_10, 1, ADC_SampleTime_55Cycles5);
